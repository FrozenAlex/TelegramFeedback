...
router.post('/', async (ctx, next) => {
    ...
    if (data.message && data.name) {
        ...
        try {
            ...
            await bot.telegram.sendMessage(adminID, Message.generateMessage(data.name, data.message), {
                parse_mode: 'Markdown'
            })
            ...
            try {
                ...
                ctx.response.body = pug.renderFile("templates/index.pug", {
                    ...
                    message: {
                        type: "error",
                        text: (process.env.NODE_ENV === "development") ? JSON.stringify(err) : Error.getErrorMessage(err.code)
                    }
                })
